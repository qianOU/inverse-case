# -*- coding: utf-8 -*-
"""
Created on Wed Nov 28 00:17:04 2018
...
def get_text(url):
    ...
    pass
def opreate_html(text):
    soup=bs(text,'lxml')
    
    with open('zhihu2.txt','a',encoding='utf-8') as f:
        for i in soup.find_all(name='div',attrs={'class':'explore-feed feed-item'}):
          try:
            a1='data:'+i['data-offset']
            a2='title:'+i.h2.string.strip()
            a3=''
            q=i.find(name='textarea',attrs={'class':'content'})
            z=str(q)
            z=z.replace('&lt;','<')
            z=z.replace('&gt;','>')
            w=bs(z,'lxml')
            print(w.prettify())
            for j  in w.html.body.textarea.children:
                print(4)
                a3=a3+(j.string.strip() if j.string is not None else '')
