...
class Text:
	def __init__(self, param='34748'):
		...
		self.url = 'http://www.scicol.shu.edu.cn/Default.aspx?tabid=34449'
		self.headers = {
	...
	def to_list(self):
		for i in re.findall(r'<a\sid=.*?title="(.*?)"\shref="(.*?)".*?class="linkfont1">(.*?)<', self.text, re.S):
			...
			if any([t in i[0].lower() for t in KEY_WORDS]):
				...
				# print(temp)
				self.url_list.append((i[0], temp, i[2]))#i[0]为标题,i[2]为发布时间
				self.list_name.append(i[0])
		print('第一步完成！，数学系新闻所有链接收集完成')
...
def sci_news_main():
	...
	df.to_excel(excel_writer=r'./documents/综合新闻/理学院获取的有关数学新闻信息.xlsx',index = False)
	df = pd.Series(w.list_name)
	df.to_excel(excel_writer=r'./documents/综合新闻/理学院的有关数学新闻列表.xlsx',index = False)
