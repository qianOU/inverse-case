...
class Text:
	def __init__(self, param='34748'):
		...
		self.url = 'http://www.scicol.shu.edu.cn/Default.aspx?tabid=34448'
		self.headers = {
...
def sci_research_main():
	...
	df.to_excel(excel_writer=r'./documents/科研动态/理学院获取的有关数学科研动态信息.xlsx',index = False)
	df = pd.Series(w.list_name)
	df.to_excel(excel_writer=r'./documents/科研动态/理学院的有关数学科研动态通知列表.xlsx',index = False)
