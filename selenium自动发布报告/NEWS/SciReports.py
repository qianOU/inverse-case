...
class Text:
	def __init__(self, param='34748'):
		...
		self.url = 'http://www.scicol.shu.edu.cn/Default.aspx?tabid=34784'
		self.headers = {
...
def sci_report_main():
	...
	df.to_excel(excel_writer=r'./documents/通知公告/理学院获取的有关数学通知信息.xlsx',index = False)
	df = pd.Series(w.list_name)
	df.to_excel(excel_writer=r'./documents/通知公告/理学院的有关数学通知列表.xlsx',index = False)
